...
class GameScene: SKScene {
    ...
    var lives = 5
    let trainCountMax = 15
    var gameOver = false
    let zPositionEnemyCat: CGFloat = 50.0
    ...
    let catCountLabel = SKLabelNode(fontNamed: "Chalkduster")
...
//        livesLabel.position = CGPoint.zero
        ...
        livesLabel.position = CGPoint(x: -playableRect.size.width/2 + CGFloat(100),
                                      y: -playableRect.size.height/2 + CGFloat(110))
        cameraNode.addChild(livesLabel)
        
        catCountLabel.text = "Cats: X"
...
//        livesLabel.position = CGPoint.zero
        ...
        catCountLabel.position = CGPoint(x: playableRect.size.width/2 - CGFloat(100),
                                      y: -playableRect.size.height/2 + CGFloat(110))
...
//        }
    ...
    func spawnEnemy() {
        ...
        enemy.zPosition = self.zPositionEnemyCat
        addChild(enemy)
    ...
    func spawnCat() {
        ...
        cat.position = CGPoint(
            ...
            y: CGFloat.random(min: self.cameraRect.minY,
                              max: self.cameraRect.maxY))
        cat.zPosition = self.zPositionEnemyCat
        cat.setScale(0)
        addChild(cat)
        // 2
    ...
    func moveTrain() {
        ...
        if trainCount >= self.trainCountMax && !self.gameOver {
            self.gameOver = true
            print("You win!")
            backgroundMusicPlayer.stop()
    ...
    func backgroundNode() -> SKSpriteNode {
        ...
        let tallestBackgroundHeight = max(background1.size.height, background2.size.height)
        if tallestBackgroundHeight > 0 {
            let scale = size.height / tallestBackgroundHeight
            background1.setScale(scale)
            background2.setScale(scale)
            background2.position = CGPoint(x: background1.size.width, y: 0)
        }
        
        let yOffset = (size.height - background1.size.height) / 2
        background1.position.y = yOffset
        background2.position.y = yOffset
